 ...
 * 
 * Modification:
 *              Entend to support the mapping model that maps application to machine
 * Programmer:  Ivy Peng
 * Date:        Nov, 2017
 *
 * Modification:
 *              Entend to support comma separted trait lists
 * Programmer:  Ivy Peng
 * Date:        Nov, 2017
 *
 * Modification:
 *              Entend to support conectivity among machine components
 * Programmer:  Ivy Peng
 * Date:        Apr, 2018
 *
// *****************************************************************************/
...
connect
: TKW_CONNECT TLBRACE identlist TRBRACE { $$ = new ASTMachConnectivity($3); }
;
...
| TKW_MODEL TIDENT TLBRACE globalstatements mapdata  TRBRACE
{
  $$ = new ASTMapModel($2, $4, $5, NULL);
  $4.release();
}
| TKW_MODEL TIDENT TLBRACE globalstatements mapkernel  TRBRACE
{
  $$ = new ASTMapModel($2, $4, NULL, $5);
  $4.release();
}
;
...
| TIDENT TLPAREN arguments TRPAREN { $$ = new ASTTrait($1, $3); }
;
